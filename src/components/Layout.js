...
export class Layout extends Component {
  ...
  createSecondSelect() {
    ...
    return <select onChange={this.handleSecondSelect} value={secondCurrency}>
  ...
  render() {
    ...
    return <div>
      <select onChange={this.handleFirstSelect} value={firstCurrency}>
        ...
        {Object.keys(currencies).map(currency => <option key={currency} value={currency}>{currencies[currency]}</option>)}
      </select>
      ...
      <br />
      {firstCurrency && secondCurrency && <button onClick={() => dispatch({ type: REVERSE_CURRENCY })}>Reverse</button>}
      <ResultBox rate={rate}
                 date={date}
...
export default connect(mapStateToProps, { getCurrencies, getCurrentFXRate, getFXRateByDate })(Layout);